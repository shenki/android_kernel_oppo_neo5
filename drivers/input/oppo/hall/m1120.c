...
#else
#define M1120_EVENT_CODE					KEY_F1
#endif
...
static short m1120_lthd_gain = 0;//-3
static short m1120_hthd_gain = 8;//-1
...
//static unsigned int DHALL_IRQ_GPIO = 0xFFFF;
...
static int m1120_set_detection_mode(struct device *dev, u8 mode)
{
	...
	if(mode & M1120_DETECTION_MODE_INTERRUPT) {
		...
		if(!p_data->irq_enabled) {
			...
			enable_irq_wake(p_data->irq);//enable_irq(p_data->irq);
...
static void m1120_get_adc_datas(short *adc_max, short *adc_min, int count)
{
    ...
    //m1120_power_up(&p_m1120_data->client->dev);
...
static int m1120_set_calibration(struct device *dev)
{
    ...
    //p_data->thrlow = min + m1120_lthd_gain;
    p_data->thrhigh = max + m1120_hthd_gain;
                              ...
                              struct device_attribute *attr, char *buf)
{
    ...
    err = m1120_measure(p_data, &adc);
    //}
    //err = m1120_get_calibrated_data(dev,&adc);
    return sprintf(buf, "%d\n", adc);
}
...
/*
	if(p_platform) {
		...
		p_data->irq			= client->irq;
	}
	else {
              printk("%s  get platform data 1111.... \n", __FUNCTION__);
		p_data->power_vi2c = -1;
		p_data->power_vdd = -1;
		p_data->igpio = -1;
	}
